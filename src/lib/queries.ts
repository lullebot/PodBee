...
async function loadCredits(
  ...
  const { data } = await supabase
    .from(table)
    .select(
      "role_id, billing_order, people(id, slug, display_name, image_url), credit_roles(label)"
    )
    .eq(fk, id)
    .order("billing_order");
...
export async function getPodcastDetail(
  ...
  const [{ data: primary_company }, { data: seasons }, { data: episodes }] =
    await Promise.all([
  ...
  const [{ data: podcastCredits }, { data: episodeCredits }] =
    await Promise.all([
      supabase
        ...
        .select(
          "role_id, billing_order, credit_roles(label), podcasts(id, slug, title, cover_image_url)"
        )
        .eq("person_id", pe.id)
        .order("billing_order"),
      supabase
        ...
        .select(
          "role_id, billing_order, credit_roles(label), episodes(id, slug, title, cover_image_url, published_at, podcast_id, podcasts(id, slug, title, cover_image_url))"
        )
        .eq("person_id", pe.id)
        .order("billing_order"),
    ]);
